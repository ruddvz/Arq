...
describe('generateWallOpeningMeshes', () => {
  it('produces exactly one mesh (the whole wall) when there are no openings', () => {
    const meshes = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [], tolerance);
    expect(meshes).toHaveLength(1);
  });

  it('produces pier + header for a door (zero sillHeight) not touching either end', () => {
    const door: WallOpeningSpan = { offsetFromWallStart: 4, width: 0.9, sillHeight: 0, height: 2.1 };
    const meshes = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [door], tolerance)!;
  ...
  it('produces pier + sill + header for a window (non-zero sillHeight, top below wall height)', () => {
    const window: WallOpeningSpan = { offsetFromWallStart: 4, width: 1.2, sillHeight: 0.9, height: 1.2 };
    const meshes = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [window], tolerance)!;
    // left pier, right pier, sill, header = 4 panels.
    expect(meshes).toHaveLength(4);
  });

  it('omits the left pier when the opening touches the wall start', () => {
    const door: WallOpeningSpan = { offsetFromWallStart: 0, width: 0.9, sillHeight: 0, height: 2.1 };
    const meshes = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [door], tolerance)!;
  ...
  it('omits the right pier when the opening touches the wall end', () => {
    const door: WallOpeningSpan = { offsetFromWallStart: 9.1, width: 0.9, sillHeight: 0, height: 2.1 };
    const meshes = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [door], tolerance)!;
    expect(meshes).toHaveLength(2);
  });

  it('adds a middle pier between two non-touching openings', () => {
    const doorA: WallOpeningSpan = { offsetFromWallStart: 1, width: 0.9, sillHeight: 0, height: 2.1 };
    const doorB: WallOpeningSpan = { offsetFromWallStart: 5, width: 0.9, sillHeight: 0, height: 2.1 };
  ...
  it('is order-independent for the input openings list (sorts internally)', () => {
    const doorA: WallOpeningSpan = { offsetFromWallStart: 1, width: 0.9, sillHeight: 0, height: 2.1 };
    const doorB: WallOpeningSpan = { offsetFromWallStart: 5, width: 0.9, sillHeight: 0, height: 2.1 };
    const forward = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [doorA, doorB], tolerance)!;
    const reversed = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 0, [doorB, doorA], tolerance)!;
  ...
  it('returns null for a zero-length centerline (degenerate wall)', () => {
    ...
    expect(generateWallOpeningMeshes(degenerate, thickness, 'centre', wallHeight, 0, [], tolerance)).toBeNull();
  });

  it('returns null for a non-positive wallHeight', () => {
    expect(generateWallOpeningMeshes(centerline, thickness, 'centre', 0, 0, [], tolerance)).toBeNull();
    expect(generateWallOpeningMeshes(centerline, thickness, 'centre', -1, 0, [], tolerance)).toBeNull();
  });

  it('returns null for a non-finite wallHeight (adversarial: non-finite values)', () => {
    expect(generateWallOpeningMeshes(centerline, thickness, 'centre', Number.NaN, 0, [], tolerance)).toBeNull();
  });

  it('places panels at the given baseElevation', () => {
    const meshes = generateWallOpeningMeshes(centerline, thickness, 'centre', wallHeight, 3, [], tolerance)!;
    const mesh = meshes[0]!;
