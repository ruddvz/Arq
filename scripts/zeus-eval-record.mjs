...
if (!new Set(['green','partial','blocked','failed','rolled_back']).has(entry.outcome)) errors.push('invalid outcome');
if (!entry.task || entry.task.length > 180) errors.push('task required and <=180 chars');
if (!Number.isInteger(entry.rounds) || entry.rounds < 0 || entry.rounds > 20) errors.push('invalid rounds');
const serialized=JSON.stringify(entry); if(/gh[pousr]_[A-Za-z0-9]{20,}|(?:secret|token|password|api[_-]?key)\s*[=:]/i.test(serialized)) errors.push('entry appears to contain a secret');
if (errors.length) {
