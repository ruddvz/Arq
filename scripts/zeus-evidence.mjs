...
const compactGraph = (graph, tier) => ({
  tier,
  fresh: graph.fresh,
  uncertainty: Boolean(graph.uncertainty),
  reason: graph.reason,
...
const currentGraph = (ledger) => {
  ...
  const seeds = recorded?.seeds?.requested ?? [];
...
if (sub === 'init') {
  ...
  save({
    ...
    repositoryIntelligence: graph ? compactGraph(graph, tier) : null,
    entries: [],
    openConflicts: [],
  });
  ...
  if (graph && entry.state === 'verified' && !graph.fresh) {
    console.error(
      `Cannot record graph-derived verified evidence while repository graph is stale (${graph.reason}). Rebuild and inspect the graph first.`,
    );
    process.exit(2);
  }
  if (graph?.fresh) {
    const tier = ledger.repositoryIntelligence.tier ?? 'standard';
    ledger.repositoryIntelligence = compactGraph(graph, tier);
    entry.repositoryGraph = {
      fingerprint: graph.fingerprint,
  ...
  if (graph) {
    if (!graph.fresh) {
      graphProblems.push(`repository graph is stale (${graph.reason})`);
    } else if (graph.fingerprint !== ledger.repositoryIntelligence.fingerprint) {
      graphProblems.push('repository graph fingerprint changed since the last evidence record');
    }
    if (graph.fresh) {
      for (const entry of ledger.entries.filter((item) => item.state === 'verified')) {
        if (entry.repositoryGraph?.fingerprint !== graph.fingerprint) {
          graphProblems.push(`verified claim is not bound to the current graph: ${entry.claim}`);
        }
      }
    }
  }
