...
async function runProbe(browser, origin, forcedColors) {
  ...
  const result = await page.evaluate(async () => {
    ...
    host.style.cssText =
      'position:fixed;inset:120px 120px 120px 120px;z-index:999998;pointer-events:none;overflow:hidden;background:repeating-linear-gradient(45deg,#183a32 0 18px,#d9e8e2 18px 36px)';
    const moving = document.createElement('div');
    moving.style.cssText =
      'position:absolute;inset:-30%;background:repeating-radial-gradient(circle,#0b6b50 0 8px,#f6efe4 8px 20px);will-change:transform';
    ...
    await new Promise((resolve) => {
      ...
      const tick = (now) => {
        ...
        previous = now;
        frame += 1;
        if (frame < 201) requestAnimationFrame(tick);
        else resolve();
      };
...
async function main() {
  if (!existsSync(path.join(distDir, 'index.html')))
    throw new Error('apps/web/dist is absent. Build the production web app first.');
  ...
  try {
    const states = {};
    for (const [name, forcedColors] of [
      ['default', false],
      ['forced-colors', true],
    ]) {
      const runs = [];
    ...
    const file = path.join(
      outDir,
      `material-compositing-${new Date().toISOString().replace(/[:.]/g, '-')}.json`,
    );
    writeFileSync(
      file,
      `${JSON.stringify({ ...report, budget: null, enforcement: workflow.enforcement }, null, 2)}\n`,
    );
