...
export function validateAutonomy(value) {
  ...
  if (value?.runtimeDependenciesOnPeers !== false) errors.push('runtimeDependenciesOnPeers must remain false')
  if (value?.sharedMutableState !== false) errors.push('sharedMutableState must remain false')
  if (value?.federation?.mode !== 'reviewed-knowledge-only') errors.push('federation mode must remain reviewed-knowledge-only')
  for (const key of REQUIRED_LOCAL_AUTHORITIES) {
    if (value?.localAuthority?.[key] !== true) errors.push(`localAuthority.${key} must remain true`)
  }
...
export function validateAutonomyFiles(root = process.cwd()) {
  ...
  try {
    errors.push(...validateAutonomy(JSON.parse(readFileSync(join(root, '.zeus/autonomy.json'), 'utf8'))))
  } catch (error) {
  ...
  if (!agents.includes('.zeus/AUTONOMY.md')) errors.push('AGENTS.md must conditionally route federation work through .zeus/AUTONOMY.md')
  return errors
}
