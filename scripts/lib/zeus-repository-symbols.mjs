...
function stripRustSymbol(pathValue) {
...
export function rustModuleCandidates(sourceRel, rustPath) {
  const raw = rustPath.replace(/^::/, '');
  const normalized = stripRustSymbol(raw);
...
export function rustUseModuleCandidates(sourceRel, rustPath) {
  const direct = rustModuleCandidates(sourceRel, rustPath);
  const raw = rustPath.replace(/^::/, '');
  const parts = raw.split('::').filter(Boolean);
  if (parts.length === 2 && ['crate', 'self', 'super'].includes(parts[0])) {
    const sourceParts = sourceRel.replaceAll('\\', '/').split('/');
    const srcIndex = sourceParts.lastIndexOf('src');
    if (srcIndex < 0) return direct;
    const crateRoot = sourceParts.slice(0, srcIndex + 1).join('/');
    const sourceDir = sourceParts.slice(0, -1).join('/');
    const baseDir = parts[0] === 'crate' ? crateRoot : sourceDir;
    const base = `${baseDir}/${parts[1]}`;
    return [...new Set([...direct, `${base}.rs`, `${base}/mod.rs`])];
  }
  return direct;
}
