...
if (!runId) {
  console.error('Usage: zeus-ci-triage --run <workflow-run-id>');
  process.exit(2);
}
const gh = (a) => spawnSync('gh', a, { encoding: 'utf8', maxBuffer: 20 * 1024 * 1024 });
if (gh(['--version']).status !== 0) {
  console.error('gh CLI unavailable');
  process.exit(3);
}
const view = gh([
  'run',
  ...
  runId,
  '--json',
  'databaseId,workflowName,status,conclusion,headSha,url,jobs',
]);
if (view.status !== 0) {
...
const logs = gh(['run', 'view', runId, '--log-failed']);
...
const patterns = [
  [
    ...
    /resource not accessible|permission denied|bad credentials|secret.*not found|403/i,
  ],
  [
    'dependency-or-supply-chain',
    /npm audit|pnpm.*ERR|lockfile|integrity checksum|vulnerab|license/i,
  ],
  ...
  JSON.stringify(
    {
      ...
      rerunAdvice: classifications.includes('infrastructure-or-flaky')
        ? 'One evidence-based rerun may be reasonable'
        : 'Fix root cause before rerun',
      failedLogExcerpt: text.slice(-12000),
    },
    null,
    2,
  ),
