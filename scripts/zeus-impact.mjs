...
const map = JSON.parse(
  readFileSync(join(packageRoot, '.zeus', 'impact-map.json'), 'utf8'),
);
...
const radiusRank = Object.fromEntries(
  blastRadius.levels.map((l) => [l.id, l.rank]),
);
const reversibilityRank = Object.fromEntries(
  blastRadius.reversibility.map((r) => [r.id, r.rank]),
);
...
if (task) {
  ...
  if (
    reversibilityRank[r.reversibility] > reversibilityRank[reversibility]
  ) {
    reversibility = r.reversibility;
  }
}
...
if (radiusRank[blast] >= radiusRank.persistent && rank[risk] < rank.high) {
  risk = 'high';
}
...
const graph = files.length
  ? repositoryEvidence(root, files, fromTask?.tier ?? 'standard')
  : null;
if (graph) {
