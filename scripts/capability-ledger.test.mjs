...
function record(capabilityId) {
  const found = ledger().capabilities.find((item) => item.capabilityId === capabilityId);
...
describe('ARQ capability ledger', () => {
  ...
  it('seeds every required Core 1.0 capability family', () => {
    ...
    const ids = new Set(ledger().capabilities.map((item) => item.capabilityId));
  ...
  it('makes maturity weights explicit and reproducible', () => {
    ...
    const expected =
      output.capabilities.reduce((sum, capability) => sum + capability.maturityWeight, 0) /
