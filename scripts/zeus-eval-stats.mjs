...
for (const line of lines) { try { entries.push(JSON.parse(line)); } catch { malformed += 1; } }
if (!entries.length) {
  console.log(`No valid Zeus entries${malformed ? `; ${malformed} malformed` : ''}.`);
  process.exit(malformed ? 1 : 0);
}
const by=(field)=>entries.reduce((acc,item)=>{const key=item[field]??'unknown';acc[key]=(acc[key]??0)+1;return acc;},{});
console.log(`Zeus evaluations: ${entries.length}${malformed ? ` (${malformed} malformed)` : ''}`);
console.log(`Outcomes: ${Object.entries(by('outcome')).map(([k,v])=>`${k}=${v}`).join('  ')}`);
console.log(`Lanes: ${Object.entries(by('lane')).map(([k,v])=>`${k}=${v}`).join('  ')}`);
