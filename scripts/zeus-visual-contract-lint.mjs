...
if (!path) {
  console.error('Usage: zeus-visual-contract-lint <spec.md>');
  process.exit(2);
}
...
const missing = required.filter(([, r]) => !r.test(text)).map(([n]) => n);
if (missing.length) {
  console.error(`Visual contract incomplete: ${missing.join(', ')}`);
  process.exit(1);
}
console.log('Visual contract lint passed.');
