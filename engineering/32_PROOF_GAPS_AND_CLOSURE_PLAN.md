...
| Pages truth proof          | Static Pages builds and deploys.                                                          | Install Language System 4.1 scripts and add artifact plus live proof jobs.                                                 | `language_contract`, `rendered_public_site`, and `deployed_public_site` pass for the same commit.                             | Block release of material marketing claims.                     |
...
| CI default-branch coverage | Existing `ci.yml` pushes to `main` while the audited default branch differs.              | Make trigger and required-check coverage follow the actual default branch, and add `merge_group` before merge queue use.   | A shadow run proves both PR and merge-group paths.                                                                            | Do not rely on current push coverage for default-branch policy. |
| Source ownership           | CODEOWNERS contains placeholders.                                                         | Assign real teams or people for document, file-format, geometry, security, and control-plane paths.                        | No placeholder entries and a review test or manual confirmation.                                                              | Do not claim code-owner enforcement is active.                  |
